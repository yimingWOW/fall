...
use crate::constants::{ LENDING_AUTHORITY_SEED, BORROWER_AUTHORITY_SEED, BORROW_TOKEN_SEED, COLLATERAL_TOKEN_SEED, BORROWER_BORROW_BLOCK_HEIGHT_TOKEN_SEED,LENDING_SEED };
...
pub struct Borrow<'info> {
    ...
    #[account(
        mut,
        associated_token::mint = lending_pool.mint_b,
        associated_token::authority = borrower,
    )]
...
pub fn borrow(
    ...
    require!(
        ctx.accounts.lending_pool.check_collateral_ratio(
...
#[error_code]
pub enum BorowError {
    #[msg("Borrower already has a borrow")]
    Error10,
}